...
# === 3.5 Conta e filtra gruppi con almeno 2 persone ===
group_sizes = df['Group'].value_counts()
multi_member_groups = group_sizes[group_sizes >= 2].index
df = df[df['Group'].isin(multi_member_groups)]
...
# === 6. Output ===
print("\nAnalisi gruppi con almeno 2 persone e unico cognome:")
print(f"Gruppi totali: {total_groups}")
print(f"Gruppi con un solo cognome: {coherent_groups}")
print(f"Percentuale: {percentage:.2f}%")

# === 7. Analisi dei gruppi non coerenti con più cognomi ===
non_coherent_multi_groups = surname_counts_per_group[surname_counts_per_group > 1].index
...
print(f"\nGruppi NON coerenti (più cognomi): {total_noncoherent_multi}")
...
plt.title('Distribuzione gruppi (≥2 persone) per coerenza del cognome')
...
if total_noncoherent_multi > 0:
    ...
    plt.title('Nei gruppi non coerenti (≥2 persone): esiste un cognome prevalente?')
